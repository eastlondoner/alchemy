...
export async function createTunnel(
  ...
  const extractNamedTunnelUrl = (line: string) => {
    if (line.includes("Generated Connector ID")) {
      console.log(
        "tunnelProps?.tunnel.dnsRecords",
        tunnelProps?.tunnel.dnsRecords,
      );
      const dnsRecords = tunnelProps?.tunnel.dnsRecords
        ? Object.entries(tunnelProps?.tunnel.dnsRecords)
        : [];
      if (dnsRecords.length > 1) {
        console.warn(
          "Multiple DNS records found for tunnel, using the first one",
        );
      }
      const ingressRules = tunnelProps?.tunnel.ingress;
      if (!ingressRules || ingressRules.length < 2) {
        throw new Error(
          "Dev tunnels must have at least one ingress rule pointing to localhost:target_port",
        );
      }
      ...
      for (const rule of ingressRules) {
        ...
        if (
          rule.service &&
          !rule.service.startsWith("http://localhost:") &&
          !rule.service.startsWith("tcp://localhost:")
        ) {
          throw new Error(
            `Dev tunnels must route to localhost, but ${rule.service} was found`,
          );
        }
        if (
          rule.service &&
          rule.service.startsWith(`http://localhost:${tunnelProps!.port!}`)
        ) {
      ...
      if (!matchingServiceFound) {
        throw new Error(
          `Dev tunnels must have at least one ingress rule with a service routing to localhost:${tunnelProps!.port!}, but no matching ingress rule was found`,
        );
      }
      const argoTunnelHostname = `${tunnelProps?.tunnel.tunnelId}.cfargotunnel.com`;
      const hostname =
        dnsRecords.length > 0 ? dnsRecords[0][0] : argoTunnelHostname;
      return `https://${hostname}`;
    }
  };

  const cmd =
    tunnelMode === "NAMED_TUNNEL"
