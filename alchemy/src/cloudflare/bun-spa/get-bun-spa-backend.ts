...
export function getBackendUrl({ routePath }: GetBackendUrlProps = {}): URL {
    let pathToUse = !routePath ? '/' : routePath.startsWith('/') ? routePath : `/${routePath}`;
    // This is ugly but it's necessary to support both local and prod environments
    // Start with the current host
    let backendUrl: string = window.location.protocol + '//' + window.location.host + pathToUse;
    try {
    ...
    console.log("Using apiBaseUrl", backendUrl);
