...
Enable quick tunnel mode to automatically proxy your worker's public URL through a tunnel:
...
To use custom hostnames to route public traffic to your local development environment with tunnels use the [`DevTunnel`](/providers/cloudflare/dev-tunnel/) resource:

```ts
import { DevTunnel, Worker } from "alchemy/cloudflare";

// Create a named tunnel with specific hostnames that you want to use. You can only use hostnames on Cloudflare managed domains in your account.
const devTunnel = await DevTunnel("dev-tunnel", {
  name: "my-dev-tunnel",
  hostnames: ["api-dev.example.com", "web-dev.example.com"] as const,
});
...
const apiWorker = await Worker("api", {
  ...
  dev: {
    tunnel: devTunnel.addRoute("api-dev.example.com"),
  },
});

// Web worker on web-dev.example.com
const webWorker = await Worker("web", {
  entrypoint: "./src/web-worker.ts",
  bindings: {
    API: apiWorker, // Service binding works locally
  },
  dev: {
    tunnel: devTunnel.addRoute("web-dev.example.com"),
  },
});
```

Named tunnels provide:
- **Custom hostnames**: Use your own domains for development
- **Deployed Workers unaffected**: Deployed workers are not modified and continue to server traffic as normal
- **Automatic DNS**: DNS records & SSL certificates are created for each hostname automatically
- **Stable URLs**: Persist across development sessions

See the [DevTunnel documentation](/providers/cloudflare/dev-tunnel/) for more details.
